...
@SpringBootTest
@Transactional
class CloseReviewGroupUseCaseTest {
    ...
    fun `진행 중인 그룹을 마감한다`() {
        ...
        assertEquals(ReviewGroupStatus.CLOSED, reviewGroupRepository.findById(reviewGroup.id!!).get().status)
    }

    @Test
    fun `진행 중이 아닌 그룹을 마감하면 예외가 발생한다`() {
    ...
    @Test
    fun `존재하지 않는 그룹을 마감하면 예외가 발생한다`() {
        assertFailsWith<ReviewGroupNotFoundException> {
            closeReviewGroupUseCase.close(-1L)
        }
    }
}
